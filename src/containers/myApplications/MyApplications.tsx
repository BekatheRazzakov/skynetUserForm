...
const MyApplications = () => {
  ...
  return (
    <Paper
      className='my-applications-container'
      elevation={4}
    >
      <ApplicationPopup
        togglePopup={toggleAppPopup}
        popup={applicationPopup}
      >
        <Typography
          variant='body1'
          className='application-popup-title'
        >Информация о заявке</Typography>
        <Paper
          elevation={1}
          className='application-item-content'
        >
        ...
        <Paper
          ...
          className='application-item-content'
        >
          <span className='application-item-key-name'>Дата создания: </span>
          <span className='application-item-key-value'>{application?.created_at ? formatDate(application?.created_at, true) : '-'}</span>
        </Paper>
      </ApplicationPopup>
      {
        applicationsLoading ?
            ...
            <div className='application-items'>{
              applications.map((app, i) => (
                ...
                >
                  ...
                  <div className='application-item-content'>
                    <span className='application-item-key-name'>Дата создания: </span>
                    <span className='application-item-key-value'>{app?.created_at ? formatDate(app.created_at, true) : '-'}</span>
                  </div>
                </Paper>
              ))
            }</div>
